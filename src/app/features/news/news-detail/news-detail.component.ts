...
export class NewsDetailComponent implements OnInit {
  ...
  loadArticle(): void {
    ...
    setTimeout(() => {
      const article = this.getMockArticle(this.articleId);
  ...
  loadRelatedArticles(category: string): void {
    ...
    this.relatedArticles = allArticles
      .filter(a => a.category === category && a.id !== this.articleId)
      .slice(0, 3);
  }

  getMockArticle(id: number): NewsArticle | undefined {
    return this.getMockArticles().find(a => a.id === id);
  }

  getMockArticles(): NewsArticle[] {
    // Mock data for news articles (same as in news.component.ts)
    return [
      {
        id: 1,
  ...
  formatContentWithParagraphs(content: string): string[] {
